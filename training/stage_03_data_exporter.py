...
class DataExporter:
    def __init__(self, config, logger, is_log_enable):
        try:
            ...
            self.master_csv=self.config['artifacts']['trainig_data']['master_csv']
        except Exception as e:
            generic_exception=GenericException(
                'Error occured in module [{0}] class [{1}] method [{2}]'
                .format(self.__module__,self.DataExporter.__name__,
...
def export_main(config_path: str, is_logging_enable=True, execution_id=None, executed_by=None)->None:
    try:
        ...
        logger.log("Database has been successfully exported in directory and exiting export pipeline.")
    except Exception as e:
        generic_exception=GenericException(
            "Error occured in module [{0}] method [{1}]"
            .format(export_main.__module__, export_main.__name__))
        raise Exception(generic_exception.error_message_detail(str(e),sys)) from e
