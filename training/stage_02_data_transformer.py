...
from mongo_db.mongo_db_atlas import MongoDBException, MongoDBOperation
...
class DataTransformer:
    def __init__(self, config, logger, is_log_enable=True):
        try:
            ...
            self.dataset_database=self.config['dataset']['detabse_detail']['training_database_name']
            ...
            generic_exception=GenericException(
                ...
                .format(self.__module__,self.DataTransform.__name__,
    ...
    def remove_unwanted_columns(self,df):
        try:
            ...
            generic_exception=GenericException(
                "Error occured in moduel [{0}] class [{1}] method [{2}]"
                .format(self.__module__, self.DataTransformer.__name__,
                self.remove_unwanted_columns.__name__))
            raise Exception(generic_exception.error_message_detail(str(e), sys)) from e 
    ...
    def unite_dataset(self):
        try:
            ...
            self.logger.log(f"Inserting dataset into database {self.dataset_database}"
                            f"collection name : {self.dataset_collection_name}")
            self.mongo_db.insert_dataframe_into_collection(self.dataset_database,self.dataset_collection_name,df)
        
        except Exception as e:
            generic_exception= GenericException(
                "Error occured in moduel [{0}] class [{1}] method [{2}]"
                .format(self.__module__, self.DataTransforme.__name__,
                self.unite_dataset.__name__))
            raise Exception(generic_exception.error_message_detail(str(e),sys)) from e

def transform_main(config_path:str, datasource:str, is_logging_enable=True,execution_id=None,
    ...
    try:
        ...
        generic_exception = GenericException(
            "Error occured in moduel [{0}] method [{1}]"
            .format(transform_main.__module__,transform_main.__name__))
        raise Exception(generic_exception.error_message_detail(str(e),sys)) from e    

if __name__=="__main__":
    args=argparse.ArgumentParser()
    args.add_argument('--config','-c', default=os.path.join('config','params.yaml'))
    args.add_argument('--datasource','-c', default=None)
